...
public class ZoomController : MonoBehaviour, IInteractable {
    ...
    public void OnTouchEnter(Vector3 touch_pos) {
        ...
        Camera.main.transform.DOMove(pos, (float)ConstantTweakTool.Instance.const_dict[kCameraEase]);
        scene_.transform.DOScale(target_.localScale,
            (float)ConstantTweakTool.Instance.const_dict[kCameraEase]);
    ...
    public virtual void OnTransitIn() {
        if (anim_obj_ != null)
            anim_obj_.DOLocalMove(move_dist_, (float)ConstantTweakTool.Instance.const_dict[kCameraEase]);
    ...
    public virtual void OnTransitOut() {
        if (anim_obj_ != null)
            anim_obj_.DOLocalMove(Vector3.zero, (float)ConstantTweakTool.Instance.const_dict[kCameraEase]);
        if (trigger_obj_ != null)
            foreach(GameObject trigger in trigger_obj_)
                trigger.SetActive(false);
    }
